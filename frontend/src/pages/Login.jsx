...
const Login = () => {
  ...
  // Where to redirect after login (e.g. user came from /appointment/...)
  ...
  const onSubmitHandler = async (event) => {
    ...
    if (!backendUrl) {
      toast.error('Backend not connected. Please check .env file.')
      return
    }
    ...
    <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center'>
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border rounded-xl text-[#5E5E5E] text-sm shadow-lg'>
        ...
        {state === 'Sign Up'
          ...
          : <p>New here?{' '}
              <span onClick={() => setState('Sign Up')} className='text-primary underline cursor-pointer'>Create an account</span>
            </p>
        }
      </div>
    </form>
